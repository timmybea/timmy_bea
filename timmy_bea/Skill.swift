...
struct Skill {
    ...
    static func skillDataArray() -> [Skill] {
        ...
        let networking = Skill(title: "Networking", underline: 106, bodyText: "I have experience performing many common networking tasks in apps. Some tasks include using APIs to download data, parsing json data for model objects, caching reusable data, using predicates in data requests, and uploading user data to a database in real time. I have built my own server using the Kitura framework, tested it locally with Docker Containers and hosted it with BlueMix.")
        ...
        let persistence = Skill(title: "Data Persistence", underline: 152, bodyText: "There are so many ways with which to persist data and I have taken the opportunity to learn several of them from user defaults, to the keychain, to using the file manager to persist data to the file system. I have also taken considerable time to familiarise myself with CoreData, and am comfortable creating managed objects and retrieving them with fetch requests, and fetched results controllers using predicates.")
